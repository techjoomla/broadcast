  ...
  function renderPluginHTML($config)
	...
	function plug_techjoomlaAPI_googleplusgetstatus()
	{
  	...
  	function renderstatus($totalresponse)
  	{
		...
		foreach($totalresponse['items'] AS $gstatus)
		{
			$status[$j]['comment'] 	=  $gstatus['title'];
			$status[$j]['timestamp'] =	strtotime($gstatus['updated']);
			//$config =JFactory::getConfig();
			//$offset = $config->get('config.offset');
			//$get_date= date('Y m d H:i:s',$status[$j]['timestamp']);
			//$status[$j]['timestamp'] = $get_date;
			$j++;
