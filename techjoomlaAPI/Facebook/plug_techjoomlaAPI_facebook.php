  ...
  function curl_get_file_contents($URL) {
	...
	{ 
		...
		$userid=$this->user->id;
			...
			$this->db->setQuery($qry);	
			$this->db->query();
			if(file_exists(JPATH_SITE.DS.'components'.DS.'com_broadcast'.DS.'views'.DS.'config'.DS.'default.php'))
			{
				...
				if (in_array('params', $field_array)) { 
					$qry 	= "UPDATE #__broadcast_config SET params='' WHERE user_id={$userid} ";
					$this->db->setQuery($qry);	
					$this->db->query();
				}
			}
	...
	{	
		...
		$raw_mail	=	 $invhelper->buildCommonPM('');
	...
	{ 
		...
		return false;
	 	foreach($oauth_keys as $oauth_key){	
			...
			try{	
			/*$checked_facebook_post_to=0;
			$post_to=combroadcastHelper::getprivacylists($oauth_key->user_id);
			if($post_to){
			$post_to_arr=json_decode($post_to->get_from,true);
			if(in_array('plug_techjoomlaAPI_facebook',$post_to_arr))
			$checked_facebook_post_to=1;
			else
				$checked_facebook_post_to=0;
			}
		if($checked_facebook_post_to)*/	
	...
	$response='';
		...
		if(isset($token))
		{
			...
			*/
			...
			/*$post_to=combroadcastHelper::getprivacylists($oauth_key[0]->user_id);
			if($post_to){
			$post_to_arr=json_decode($post_to->post_to,true);
			if(in_array('plug_techjoomlaAPI_facebook',$post_to_arr))
			$checked_facebook_post_to=1;
			else
				$checked_facebook_post_to=0;
