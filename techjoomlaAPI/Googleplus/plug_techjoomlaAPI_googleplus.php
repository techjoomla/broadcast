  ...
  function renderPluginHTML($config) 
	...
	{ 
			...
			foreach($oauth_keys as $oauth_key){			
				$oauth_token		 	= json_decode($oauth_key->token,true);
				$this->client->setAccessToken($oauth_token);

				$totalresponse = $this->plus->activities->listActivities('me','public');
										//print_r($totalresponse);die;

				if($totalresponse)
				{
						$json_linkedin= $response_updates['linkedin']; 	
						$returndata[$i]['user_id'] = $oauth_key->user_id;
						$returndata[$i]['status'] = $this->renderstatus($totalresponse);
						$i++;
  	...
  	function renderstatus($totalresponse)
  	{
			$status = array();
			$j=0;
			foreach($totalresponse['items'] as $res)
			{
				if($res['title'])
				$status[$j]['comment'] = $res['title'];
				$status[$j]['timestamp'] = strtotime($res['updated']);//format 2013-09-17T14:03:20.254Z
				$j++;
			} 
	  	return $status;
	}
	
	function plug_techjoomlaAPI_googleplussetstatus($userid,$originalContent,$comment,$attachment='',$queue='')
	{
